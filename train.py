...
from dataPreparation import loadData, preprocess, fft_process, to_generator
...
if __name__ == "__main__":
    ...
    input_shape = np.array([args.image_width, args.image_height, 2])
    ...
    if not USE_AUGMENTATION:
        ...
        model.fit(
            ...
            yTrain,
            epochs=args.epochs,
            validation_data=fft_process(to_generator(xVal, yVal, args.batch_size)),
            shuffle=True,
            callbacks=callbacks,
        )
    else:
        ...
        with tensorflow.device("/gpu:0"):
            augmented_datas_generator = datagen.flow(
                xTrain, yTrain, batch_size=args.batch_size
            )
            data_flow = fft_process(augmented_datas_generator)

            print(math.floor(xVal.shape[0] / args.batch_size))
            model.fit_generator(
                data_flow,
                validation_data=fft_process(to_generator(xVal, yVal, args.batch_size)),
                epochs=args.epochs,
                steps_per_epoch=len(augmented_datas_generator),
                validation_steps=math.floor(xVal.shape[0] / args.batch_size),
                verbose=1,
                workers=1,
                callbacks=callbacks,
    ...
    scores = model.evaluate(
        fft_process(to_generator(xTest, yTest, args.batch_size)),
        verbose=0,
        steps=math.floor(xTest.shape[0] / args.batch_size),
    )
